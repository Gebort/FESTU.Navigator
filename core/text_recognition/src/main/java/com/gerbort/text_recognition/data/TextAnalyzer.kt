package com.gerbort.text_recognition.data
...
import com.gerbort.text_recognition.domain.DetectorException
...
class TextAnalyzer: ObjectDetector {
    ...
    override suspend fun analyze(
        ...
        if (text == null) {
            return Result.failure(DetectorException.NullText)
        }
        if (text!!.textBlocks.isEmpty()) {
            return Result.failure(DetectorException.NoObjects)
        }
        val textBlock = text!!.textBlocks
            .firstOrNull { textBlock -> filterFunction(textBlock) }
            ?: return Result.failure(DetectorException.NoDigits)

        val boundingBox = textBlock.boundingBox ?: return Result.failure(DetectorException.NoBoundingBox)
